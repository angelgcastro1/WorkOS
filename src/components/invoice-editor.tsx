...
export function InvoiceEditor({
  ...
  const issuedOn = invoice?.issuedOn ?? todayIso;
    ...
    <form action={action} className="space-y-4">
          ...
          <CardContent className="grid gap-3 p-5 sm:grid-cols-2">
            ...
            <div>
              ...
              {clients.length === 0 ? (
                ...
                <select name="client_id" value={clientId} onChange={(e) => setClientId(e.target.value)} className={fieldClass}>
      ...
      {preview ? (
        <InvoicePaper
          ...
          client={
            selectedClient
              ? { name: selectedClient.name, company: selectedClient.company, email: selectedClient.email, address: selectedClient.address }
              : null
          }
          lineItems={rows}
          taxRate={taxRate}
